...
input[type="checkbox"]+label {
	...
	height: 50px;
	background:
		url(${pageContext.request.contextPath}/image/favoriteCheck/heart_off.png)
		no-repeat 0 0px/contain;
}

input[type='checkbox']:checked+label {
	background:
		url(${pageContext.request.contextPath}/image/favoriteCheck/heart_on.png)
		no-repeat 0 0px/contain;
}
...
<body>
			...
			<div class="container-fluid">
				...
				<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
					<div class="offcanvas-header">
						<h5 style="font-family: 'Rajdhani', sans-serif;" class="offcanvas-title" id="offcanvasNavbarLabel">
							Me And U<BR> MAU
						</h5>
						...
						<c:when test="${empty sessionScope.loginUser}">
							<div class="offcanvas-body">
								<ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
									<li class="nav-item"><a class="nav-link active" aria-current="page" href="${pageContext.request.contextPath}/index">홈으로</a></li>
									<li class="nav-item"><a class="nav-link" href="javascript:kakaoLogin();">로그인</a>
								</ul>
							</div>
						</c:when>
						<c:otherwise>
							<div class="offcanvas-body">
								<ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
									<li class="nav-item"><a class="nav-link active" aria-current="page" href="${pageContext.request.contextPath}/index">홈으로</a></li>
									<li class="nav-item"><a class="nav-link" href="#">${sessionScope.loginUser.userNickName}(No.${sessionScope.loginUser.userSeqId})님 환영합니다.</a></li>
									<li class="nav-item"><a class="nav-link" href="${pageContext.request.contextPath}/mypage">나의 가이드 관리</a></li>
									<li class="nav-item"><a class="nav-link" href="${pageContext.request.contextPath}/updateform?userSeqId=${sessionScope.loginUser.userSeqId}">내 정보 변경</a></li>
									<li class="nav-item"><a class="nav-link" href="${pageContext.request.contextPath}/logout">로그아웃</a></li>
									<%
									if (user.getAdminNum() == 1) {
									%>
									<li class="nav-item"><a class="nav-link" href="${pageContext.request.contextPath}/adminForm">관리자페이지</a></li>
									<%
									}
									%>
								</ul>
    ...
    -->
		...
		<div class="map_wrap">
			...
			<div id="menu_wrap">
				<!-- 지도이름 -->
				<div>
					<button class="btn btn-primary" style="border-radius: 20px; font-size: 15px; border-style: none; color: black; background-color: #F3B922; width: 100%; height: 60px; margin-top: 10px;" type="button" onclick="location.href='/mau/guideMap/<%=mapSeq%>'"><%=placegetMap.getMapIcon()%>
						<%=placegetMap.getMapName()%></button>
					<br>
					<br>
				</div>
				<!-- 지도즐겨찾기 -->
				<div style="display: flex;">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="myCheck" name="myCheck" <%if (chk != null) {%> checked <%}%>> <label for="myCheck"></label>
				</div>
			</div>
			<!-- menu_wrap end -->
			<!-- 내위치 -->
			<div class="map_leftb">
				...
				<div class="share-box">
					<div class="share-btn link" onclick="copy_url()">
						<img id="mapbutton" src="../image/map/share.png" alt="지도공유" height="50px"> <input type="hidden" name="url" id="SharePlaceUrl" value="http://localhost:8181/mau/guideMap/<%=mapSeq%>">
					</div>
				</div>
			</div>
			<!-- map_leftb end -->
			...
			<div class="map_rightb">
				<form method="post" action="${pageContext.request.contextPath}/guideMap/search/${mapSeq}">
					<div class="input-group mb-3">
						...
						<button class="btn btn-outline-secondary" id="mapbutton">
							<img id="mapbutton" src="../image/map/search.png" alt="장소찾기" height="50px">
						</button>
					</div>
				</form>
			</div>
	...
	<%-- <%=placeList.get(i).getPlaceName() %> </input>--%>
	...
	<input type="hidden" name="userSeqId" value="<%=placeList.get(i).getUserSeqId()%>">
	<!-- (가이드지도) -->
	...
	<form method="get" id="mapFavorite">
		<input type="hidden" name="mapSeq" value="<%=favorite.getMapSeq()%>"> <input type="hidden" name="userSeqId" value="<%=favorite.getUserSeqId()%>">
	</form>
	<%
	}
	%>
